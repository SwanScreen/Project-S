...
class LoginModel
{
    ...
    public function loginWithCookie()
    {
        ...
        $query = $this->db->prepare("SELECT user_id, user_email, user_password_hash, user_active,
    ...
    public function requestPasswordReset()
    {
        ...
        if ($this->setPasswordResetDatabaseToken($user_email, $user_password_reset_hash, $temporary_timestamp) == true) {
            ...
            if ($this->sendPasswordResetMail($user_email, $user_password_reset_hash, $user_email)) {
                return true;
            }
        }
        // default return
     ...
     * @return bool success status
     */
    public function sendPasswordResetMail($user_email, $user_password_reset_hash, $user_email)
    {
    ...
    public function registerNewUserWithFacebook($facebook_user_data)
    {
        ...
        $clean_user_email_from_facebook = str_replace(".", "", $facebook_user_data["username"]);
        // generate integer-timestamp for saving of account-creating date
        $user_creation_timestamp = time();

        $sql = "INSERT INTO users (user_email, user_creation_timestamp, user_active, user_provider_type, user_facebook_uid)
                VALUES (:user_email, :user_creation_timestamp, :user_active, :user_provider_type, :user_facebook_uid)";
        $query = $this->db->prepare($sql);
        $query->execute(array(':user_email' => $facebook_user_data["email"],
                              ':user_creation_timestamp' => $user_creation_timestamp,
                              ':user_active' => 1,
        ...
        $count = $query->rowCount();
        if ($count == 1) {
            $query = $this->db->prepare("SELECT user_id, user_email, user_account_type, user_provider_type
                                         FROM   users
                                         WHERE  user_email = :user_email AND user_provider_type = :provider_type");
            $query->execute(array(':user_email' => $clean_user_email_from_facebook, ':provider_type' => 'FACEBOOK'));
    ...
    public function facebookUserNameExistsAlreadyInDatabase($facebook_user_data)
    {
        ...
        $query = $this->db->prepare("SELECT user_id FROM users WHERE user_email = :clean_user_email_from_facebook");
        $query->execute(array(':clean_user_email_from_facebook' => $clean_user_email_from_facebook));
    ...
    public function generateUniqueUserNameFromExistingUserName($existing_name)
    {
    	...
    	do {
            ...
            $query = $this->db->prepare("SELECT user_id FROM users WHERE user_email = :name_with_number");
